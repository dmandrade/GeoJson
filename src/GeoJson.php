...
abstract class GeoJson implements \JsonSerializable, JsonUnserializable
{
    ...
    /**
     * Return the BoundingBox for this Dmandrade\GeoJson object.
     *
     * @return BoundingBox
     */
    ...
    /**
     * Return the CoordinateReferenceSystem for this Dmandrade\GeoJson object.
     *
    ...
    /**
     * Return the type for this Dmandrade\GeoJson object.
     *
     * @return string
     */
    ...
    final public static function jsonUnserialize($json)
    {
        if ( ! is_array($json) && ! is_object($json)) {
            throw UnserializationException::invalidValue('Dmandrade\GeoJson', $json, 'array or object');
        }

        $json = new \ArrayObject($json);

        if ( ! $json->offsetExists('type')) {
            throw UnserializationException::missingProperty('Dmandrade\GeoJson', 'type', 'string');
        }
        ...
        switch ($type) {
            ...
            default:
                throw UnserializationException::unsupportedType('Dmandrade\GeoJson', $type);
        }
        ...
        $class = sprintf('Dmandrade\GeoJson\%s\%s', (strncmp('Feature', $type, 7) === 0 ? 'Feature' : 'Geometry'), $type);
